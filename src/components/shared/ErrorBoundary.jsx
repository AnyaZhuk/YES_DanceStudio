    ...
    if (this.state.hasError) {
      return (
        <div className="flex h-full min-h-[12rem] flex-col items-center justify-center gap-4 p-6 text-center">
          ...
          <p className="max-w-md text-sm text-studio-gray dark:text-slate-400">
            {i18n.t('errors.boundaryMessage')}
          </p>
          {useMock && import.meta.env.DEV && (
            <p className="max-w-md text-xs text-studio-gray dark:text-slate-500">
              {i18n.t('errors.boundaryMockHint')}
            </p>
          )}
          <div className="flex flex-wrap items-center justify-center gap-2">
            <button type="button" onClick={this.handleRetry} className="studio-btn-ghost">
              {i18n.t('errors.retry')}
            </button>
            <button type="button" onClick={this.handleReload} className="studio-btn-primary">
